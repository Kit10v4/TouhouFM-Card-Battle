...
const SOUND_PATHS = {
    ...
    lose: '/audio/game/Lose.mp3',
    draw: '/audio/game/Draw.mp3'
};
...
function unlockAudio() {
    ...
    try {
        // Create a silent audio to trigger browser permission
        const audio = new Audio();
        audio.src = 'data:audio/wav;base64,UklGRigAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQEAAAA=';
        audio.play().then(() => {
            audioUnlocked = true;
...
function initAutoUnlock() {
  ...
  // Force unlock on any interaction with silent test
  const forceUnlock = () => {
    console.log('🔊 Force unlocking audio due to user interaction');
    unlockAudio();
    
    // Test play a silent audio to ensure unlock
    const testAudio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvGQdBDuN0fDNeSsFJHfH8N2QQAoUXrTp66hVFApGn+DyvGQdBDuN0fDNeSsF');
    testAudio.volume = 0.01;
    testAudio.play().catch(e => console.log('Silent test audio failed:', e));
  };
