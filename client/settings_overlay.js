...
(function(){
  ...
  function refreshUI(){ current = load(); bgmRange.value = Math.round(current.bgmVolume*100); sfxRange.value = Math.round(current.sfxVolume*100); bgmVal.textContent = fmtPct(current.bgmVolume); sfxVal.textContent = fmtPct(current.sfxVolume); }

  bgmRange.addEventListener('input', ()=>{ bgmVal.textContent = bgmRange.value+'%'; if(window.BGM){ BGM.setVolume(bgmRange.value/100); if(BGM.isMuted && bgmRange.value>0){ BGM.setMute(false);} } });
  sfxRange.addEventListener('input', ()=>{ sfxVal.textContent = sfxRange.value+'%'; });
  ...
  btnClose.addEventListener('click', close); dismiss.addEventListener('click', close);
